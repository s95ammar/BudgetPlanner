...
class BudgetCreateEditViewModel @ViewModelInject constructor(
    ...
    private val doesActiveBudgetExist = localRepository.doesActiveBudgetExist()
    ...
    fun onInputIsActiveStateChanged(isChecked: Boolean) {
        _activeWarningVisibility.value = isChecked && doesActiveBudgetExist && (budgetId != activeBudgetId)
    }
    ...
    private fun BudgetInputBundle.wasInactiveNowActive(id: Int): Boolean {
        val wasActive = (id == activeBudgetId)
        return !wasActive && isActive
    }

    private fun BudgetInputBundle.wasActiveNowInactive(id: Int): Boolean {
        val wasActive = (id == activeBudgetId)
        return wasActive && !isActive
    }
